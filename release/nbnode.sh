...
get_latest_version() {
    local last_version=$(curl -Ls "https://api.github.com/repos/nohara-cloud/nboard-node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [[ ! -n "$last_version" ]]; then
        log "red" "检测 Nboard Node 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 Nboard Node 版本安装"
        return 1
    fi
    echo "$last_version"
}
